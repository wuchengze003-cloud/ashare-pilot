...
import { STRATEGIES, getStrategy } from "@/lib/strategyRegistry";
import { BuySignalHistoryTable } from "./BuySignalHistoryTable";
import { EquityChart, ThemeChart } from "./Charts";
import type { DashboardData } from "./types";
import type { StrategyMeta } from "@/lib/strategyRegistry";
...
export function StrategyDetailView({ data, strategyId, strategyMeta }: StrategyDetailViewProps) {
  ...
  const history = readSignalHistorySnapshots(Number.POSITIVE_INFINITY, strategyId);
  ...
  const meta = strategyMeta ?? getStrategy(strategyId);
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
        <Link href="/dashboard" className="back-link" data-umami-event="nav-dashboard-overview">策略对比总览</Link>
        <span style={{ color: "var(--muted)" }}>/</span>
        <span style={{ fontSize: 14, color: "var(--muted)" }}>{meta?.name ?? strategyId}</span>
      </div>
          ...
          <div className="strategy-tabs" style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
            {STRATEGIES.map((s) => (
              <Link
                key={s.id}
                href={`/dashboard/${s.id}`}
                className={`strategy-tab${strategyId === s.id ? " active" : ""}`}
                style={{
                  padding: "4px 12px",
                  borderRadius: 6,
                  fontSize: 13,
                  border: "1px solid var(--border, #333)",
                  background: strategyId === s.id ? "var(--accent, #4f8cff)" : "transparent",
                  color: strategyId === s.id ? "#fff" : "var(--muted, #999)",
                  cursor: "pointer",
                  textDecoration: "none",
      ...
      {!data && (
        <div className="card" style={{ borderColor: "var(--warn)" }}>
          <strong>尚未生成 {meta?.name ?? strategyId} 策略回测数据</strong>
          <p style={{ color: "var(--muted)" }}>
            收盘后运行 <code>cd web && npm run dashboard:update</code>。
            该命令会刷新行情缓存、重建三策略模拟仓和明日计划，并写入 ignored 的{" "}
            <code>web/data/runtime/strategies/{strategyId}</code>。
          </p>
        </div>
      )}
